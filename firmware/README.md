...
直接编译仓库 `src/` 下的仿真器代码（零改动），libcurl 由
`main/curl_shim/`（esp_http_client 后端子集实现）替代。
...
- **LVGL**：组件声明 `^9.3`，组件管理器实际解析到 **9.5.0**（见
  `dependencies.lock`）；仓库根目录 macOS 仿真器使用 lvgl 子模块的
  **9.3.0**，仅作参考实现，两端 UI 代码共用同一份 `src/ui/`。
- **lv_conf 双份策略**：仿真器与固件各持一份 `lv_conf.h`（根目录 =
  SDL 32bit，`firmware/lv_conf.h` = RGB565 + libc malloc，差异详见
  `firmware/lv_conf.h` 头部注释）。修改根 `lv_conf.h` 后必须同步检查
  固件变体是否需要跟进，反之亦然。
...
启动后在 `idf.py monitor` 串口里直接输入命令（提示符 `fw> `）。
所有配置保存在 NVS 命名空间 `fwcfg`，**写入后需 `restart` 重启生效**。
...
`ha_entity_*` 实体 ID，见 `src/ha_client.h`）；没有
`wifi_ssid` 时 WiFi 不启动，UI 以 `--` 降级态运行，console 仍可用。

## M4 验收清单

- [ ] `idf.py build` 通过，链接的 `src/net_worker.c`、`ha_client.c`、
      `server_client.c`、`weather_client.c`、cJSON 均零改动
- [ ] 串口 `cfg set/list/restart` 可写读凭据，token 不回显明文
- [ ] 开机 5s 内连上 WiFi（got ip 日志）；超时不阻塞启动
- [ ] 有 token：状态栏 `LIVE`，仪表盘温/湿度、设备页与服务器页数据刷新
- [ ] 无 token：状态栏 `NO TOKEN`，服务器/天气轮询仍工作
- [ ] WiFi 断开后自动重连；HA 断线走 3→30s 退避
- [ ] 天气走 HTTPS（证书包验证）正常返回深圳实况
...
- `main/app_main.c` — 装配顺序：NVS → env → WiFi → 显示/触摸 → UI →
  等待 WiFi(5s) → curl_global_init → ha_client_init → net_worker → ui_drain
