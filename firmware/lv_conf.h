/**
 * @file lv_conf.h
 * Configuration file for v9.3.0 - FIRMWARE VARIANT (ESP32-S3)
 *
 * Derived from the repository root lv_conf.h (desktop SDL simulator)
 * so that UI code written against the simulator compiles with identical
 * feature flags on the device. Deliberate differences:
 *   - LV_COLOR_DEPTH 16 (RGB565 RGB panel instead of 32-bit SDL)
 *   - LV_USE_STDLIB_MALLOC = LV_STDLIB_CLIB: LVGL allocates through libc
 *     malloc/free. With CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=4096 the big
 *     allocations land in PSRAM. (A LV_STDLIB_BUILTIN pool is a static
 *     BSS array that is always linked into internal DRAM - a 256KB pool
 *     overflows dram0_0_seg on the S3, so the builtin pool is unusable
 *     here despite the CONFIG_SPIRAM_MALLOC_* knobs.)
 *   - LV_USE_SDL / LV_USE_TINY_TTF off, Montserrat 14/16/20 on
 *   - examples/demos off to save flash
 *
 * Wiring: included by LVGL through -DLV_CONF_PATH="<this file>" set in
 * firmware/CMakeLists.txt (CONFIG_LV_CONF_SKIP=y in sdkconfig.defaults).
 */
...
#define LV_COLOR_DEPTH 16   /* firmware: ST7701 RGB565 panel */
 ...
 */
#define LV_USE_STDLIB_MALLOC    LV_STDLIB_CLIB   /**< Firmware: use libc malloc so LVGL's big allocations go to PSRAM. A LV_STDLIB_BUILTIN pool is a static BSS array pinned to internal DRAM and overflows dram0_0_seg. */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    #define LV_MEM_SIZE (256 * 1024U)         /**< [bytes] lesson learned: a pool that is too small makes the renderer hang in an infinite loop; keep this generous. */
...
#define LV_DEF_REFR_PERIOD  15      /**< [ms] ~66 FPS — match official BSP default for smooth scrolling */
 ...
 * - LV_OS_SDL2
 * - LV_OS_CUSTOM */
#define LV_USE_OS   LV_OS_FREERTOS   /**< firmware: multi-threaded rendering (2 draw units) */
...
#define LV_USE_DRAW_SW 1
    ...
    #define LV_DRAW_SW_DRAW_UNIT_CNT    2   /**< firmware: parallel rendering on both cores */
...
#define LV_USE_FS_MEMFS 1   /* firmware: JPEG variable-source decoding */
#if LV_USE_FS_MEMFS
...
#define LV_USE_TINY_TTF 0   /* firmware: Montserrat bitmap fonts only */
#if LV_USE_TINY_TTF
...
#define LV_USE_SDL              0   /* firmware: no SDL backend */
#if LV_USE_SDL
...
#define LV_BUILD_EXAMPLES 0   /* firmware: save flash */

/** Build the demos */
#define LV_BUILD_DEMOS 0      /* firmware: save flash */
