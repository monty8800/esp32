 ...
 * through a 12KB pthread stack and silently trashes neighbouring heap
 * memory (surfaced as a TLSF assert inside an unrelated LVGL realloc).
...
{
    bool ok = server_client_fetch(&s_srv_fresh);
...
{
    bool ok = weather_client_fetch(&s_wx_fresh);

    if(!ok) {
        if(!wx_failed) {
            fprintf(stderr, "[net] weather unreachable, retrying in %u min\n",
                    WX_POLL_RETRY_MS / 60000U);
        }
        wx_failed = true;
    ...
    fprintf(stderr, "[weather] Shenzhen %.1fC code=%d hum=%d%% wind=%.1fkm/h\n",
            s_wx_fresh.temp_c, s_wx_fresh.weather_code, s_wx_fresh.hum_pct,
            s_wx_fresh.wind_kmh);

    pthread_mutex_lock(&lock);
    s_wx_fresh.seq = wx_snap.seq + 1;
    wx_snap = s_wx_fresh;
    pthread_mutex_unlock(&lock);
}
...
static void * worker_main(void * arg)
{
    ...
    while(!stopping) {
        /* Gate: skip all HTTP work until the STA actually has an IP.
         * Without this, rounds fire before WiFi is up (or never started,
         * when NVS holds no credentials) and block/assert in lwip. Backoff
         * counters are untouched so normal periods resume once online. */
        if(!wifi_sta_is_connected()) {
            if(was_online) {
                ESP_LOGI(TAG, "WiFi lost - pausing network rounds");
                was_online = false;
            }
            uint64_t wake_at = now_ms() + NET_RETRY_MS;
            pthread_mutex_lock(&lock);
            while(!stopping && now_ms() < wake_at) {
                struct timespec ts;
                clock_gettime(CLOCK_REALTIME, &ts);
                uint64_t add_ms = wake_at - now_ms();
                ...
                if(wifi_sta_is_connected()) break;      /* woke early: online */
            }
            pthread_mutex_unlock(&lock);
            continue;
        }
        if(!was_online) {
            ESP_LOGI(TAG, "WiFi up - network rounds enabled");
            was_online = true;
...
bool net_worker_start(bool ha_ready_in)
{
    ...
    /* CONFIG_PTHREAD_STACK_MIN (768 B) is the default attr stack and far
     * too small for TLS/socket usage - xTaskCreate refuses it outright
     * ("pthread: Failed to create task!"). Size the stack explicitly. */
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 12 * 1024);
    int rc = pthread_create(&worker_thread, &attr, worker_main, NULL);
    pthread_attr_destroy(&attr);
    if(rc != 0) {
        fprintf(stderr, "[net] failed to create worker thread (rc=%d)\n", rc);
        return false;
    }
    worker_started = true;
