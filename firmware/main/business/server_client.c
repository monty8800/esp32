...
static bool parse_summary(const char * json_text, server_snapshot_t * out)
{
    ...
    /* ~16KB (24 hosts x probes): must NOT live on the worker's stack -
     * a 12KB pthread stack overflows into the heap and corrupts it.
     * Only ever called from the single worker thread, so static is safe. */
    static server_snapshot_t tmp;
