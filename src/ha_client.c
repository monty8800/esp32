...
/** Copy @p src into @p dst, falling back to @p fallback when src is NULL/empty. */
static void resolve_env(char * dst, size_t dst_len, const char * env_name, const char * fallback)
{
    const char * v = getenv(env_name);
    if(v == NULL || v[0] == '\0') v = fallback;
...
/**
 * Reject values that would break the hand-built JSON payloads: quotes,
 * backslashes and control characters are not allowed anywhere in the
 * strings we splice into request bodies. The body is built without an
 * escaper, so every variable part (the entity IDs from the environment)
 * is validated with this check once at init time. The remaining body
 * parts are trusted compile-time constants (on/off, the Chinese option
 * strings), so service bodies never contain unvalidated data.
 */
static bool json_safe(const char * s)
{
    for(const char * c = s; *c != '\0'; c++) {
        unsigned char uc = (unsigned char)*c;
        if(uc == '"' || uc == '\\' || uc < 0x20) return false;
    }
    return true;
}
...
static CURL * setup_curl(struct curl_slist ** headers_out,
                         resp_buf_t * resp, const char * url)
{
...
bool ha_client_fetch_state(const char * entity_id, char * out, size_t out_len)
{
    ...
    struct curl_slist * headers = NULL;
    CURL * curl = setup_curl(&headers, &resp, url);
                            ...
                            const char * json_body)
{
     ...
     * and entity IDs that passed json_safe() in ha_client_init(), so no
