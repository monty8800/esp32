...
基于 **Waveshare ESP32-S3-Touch-LCD-4B**（480×480 方形触摸屏）的桌面智能面板固件。
...
| 页面 | 内容 |
| --- | --- |
| **事务总览**（默认首页） | **运营 + 邮件 + 开发**三块事务：今日/昨日单量与销售额、缺货（缺货+低库存）、待审邮件草稿、Mac 定时任务存活、项目数与新鲜度 —— 数据来自自建 **panel-hub**（LXC 117） |
...
- **健壮性**：WiFi 断线自动重连；HA 断线 3→30s 退避；四路轮询独立节奏（HA 3s / 服务器 15s、失败退避 60s / 天气 30min、失败 5min / **panel-hub 2min、失败退避 10min**）
